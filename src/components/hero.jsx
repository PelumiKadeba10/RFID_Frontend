function Hero(){
    return(
      <>
      <div className="grid sm:grid-cols-2 grid-cols-1 px-5">
        {/* Text-section */}
        <div className="grid grid-cols-1 justify-center p-7">
            <h1 className="font-bold text-4xl">RFID Attendance System with Web Application Integration</h1>
            <p className="pt-8 font-sans text-lg text-justify">
                Welcome to our RFID Attendance System with Web App Integration.
                Easily track attendance in real-time with live data uploads and access
                previous records. Perfect for schools, workplaces, and events, our
                system ensures accurate and efficient attendance management. 
                Experience the future of attendance today.
            </p>
        </div>

        {/* Image Section */}
        <div className="flex justify-center pb-10 pt-10">
          <img src="/pic.jpg" className="w-60 h-60"/>
        </div>
      </div>
      
      </>  
    );

}

export default Hero